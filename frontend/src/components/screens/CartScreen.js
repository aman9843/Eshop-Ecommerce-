...
const CartScreen = () => {
  ...
  const value1 = parseInt(location.search.substring(location.search.length-1,location.search.length))
  console.log(value1)
  ...
  useEffect(() => {
    if (productId) {
      dispatch(addItem(productId, qnt));
        ...
        <Col md={4}>
          <Card>
            ...
            <ListGroupItem >
              ...
              <h2>
                Subtotal Items (
                {cartItems.reduce((acc, item) => acc + item.qnt, 0)})
              </h2>
              $
              {cartItems
